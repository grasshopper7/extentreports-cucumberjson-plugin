...
public class EmbeddedProcessorTest {
	...
	/*
	 * @Test public void testProcessAttachements() throws IOException { String
	 * tempFolder = testFolder.getRoot().getAbsolutePath() + "/" + (new
	 * RandomString(10)).nextString();
	 * when(reportProperties.getReportScreenshotLocation()).thenReturn(tempFolder);
	 * 
	 * Embedded embed = new Embedded(); embed.setData(Base64.encodeFromFile(
	 * "src/test/resources/tech/grasshopper/processor/image.png"));
	 * embed.setMimeType("image/png"); embeddedProcessor.processEmbedding(embed);
	 * 
	 * assertEquals("Embedded object 'data' variable value should be empty.", "",
	 * embed.getData()); assertEquals("Embedded file is not processed.", 1,
	 * Files.list(Paths.get(tempFolder)).count()); }
	 */
